...
local follow_js = [=[
  ...
  function reload_hints(array, input, keep) {
    ...
    // *qwertyboy* Setting hints length
    for(var hintlength = 1; Math.pow(charset.length, hintlength) < length; hintlength++);
    var bestposition = 37;
    ...
    for (var i=0; i<length; i++) {
      ...
      if (!e.hint.parentNode  && !e.hint.firstchild) {
        ...
        // *qwertyboy* Alt charset
        var content = document.createTextNode(intToLabel(i, hintlength));
        ...
        // *qwertyboy* Alt charset
        e.hint.textContent = intToLabel(i, hintlength);
    ...
    // *qwertyboy* Getting back lost focus
    if (!array[lastpos]) lastpos = 0;
  ...
  function update_hints(input) {
    ...
    if (input) {
      ...
      // *qwertyboy* Week differentiation between string searching and hint typing
      if (input.match(charsetregex)) {
        keep = true;
      }
    }
    ...
    for (var i=0; i<active_arr.length; i++) {
      ...
      // *qwertyboy* Search for hints and strings together
      text_content = e.hint.textContent + " " + e.element.textContent.toLowerCase();
...
add_binds("normal", {
    ...
    -- *qwertyboy* Better open in new windows and push them to background with xdotool
    buf("^F$",  function (w,b,m) w:start_follow("uri", (m.count and "open "..m.count.." tab(s)") or "open window",
                    function (uri, s)
                        for i=1,(s.count or 1) do
                            window.new{uri}
                            luakit.spawn("xdotool key --delay 300 super+Return")
                        end
                        return "root-active"
                    end, m.count) end),
    ...
    enter = function (w)
        ...
        for k, v in pairs(follow.get_theme()) do
            if type(v) == "number" then
                table.insert(js_blocks, string.format("%s = %s;", k, v))
            else
